...
class GameCard(QFrame):
    ...
    def launch_game(self):
        ...
        if not os.path.exists(main_py):
            QMessageBox.warning(self, 'Launch Error', f"No main.py found in {self.meta.get('path')}")
            return
        ...
        # reward coins if enabled
        if settings.get('mini_rewards', True):
            settings['coins'] = settings.get('coins', 0) + 1  # 1 coin per play

        save_settings()

        # check achievements based on total plays
        total_plays = sum(settings.get('play_counts', {}).values())
        if total_plays >= 1:
            check_unlock_achievement('first_play')
        if total_plays >= 5:
            check_unlock_achievement('five_plays')
        if total_plays >= 10:
            check_unlock_achievement('ten_plays')
